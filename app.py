...
st.title("🚀 Alpha-Trade Pro: Final Master Version")
...
if not data.empty and 'Close' in data.columns:
    ...
    m = Prophet(daily_seasonality=True).fit(data[['Date', 'Close']].rename(columns={'Date': 'ds', 'Close': 'y'}))
    st.line_chart(m.predict(m.make_future_dataframe(periods=30)).set_index('ds')['yhat'].tail(30))
    
    # Table
    st.subheader("Global Top 100")
    try:
        AgGrid(pd.DataFrame(requests.get("https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=100&page=1").json())[['name', 'current_price', 'market_cap_change_percentage_24h']])
    except: pass
else:
