...
# 1. Assets Definition (Crypto, AI, Commodities)
ticker_map = {
...
# 2. Advanced Data Loading with Error Handling
@st.cache_data
def load_full_data(ticker, interval):
...
data = load_full_data(ticker_map[asset], tf)
...
if not data.empty and 'Close' in data.columns:
    ...
    with col2:
        ...
        st.write(f"**RSI Value:** {data['RSI'].iloc[-1]:.2f}")
        if st.button("Send Telegram Alert"): st.success("Signal sent to Telegram!")
    ...
    # 6. Global Top 100 Data
    st.subheader("🌍 Live Top 100 Crypto Market")
    try:
        url = "https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=100&page=1"
        AgGrid(pd.DataFrame(requests.get(url).json())[['name', 'symbol', 'current_price', 'market_cap_change_percentage_24h']])
    except: st.error("Market data temporarily offline.")
else:
    st.error("Data loading failed. Please check asset or connection.")
